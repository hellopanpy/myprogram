...
class RedisServer(object):
    ...
    @timelimited(2)
    def get_key(self):
        r = redis.Redis(host=self.addr,port=self.port,password=self.password)
        key = r.get('zabbix_monitor')
        if key == 'ok':
            return True
    @timelimited(2)
    def set_key(self):
        r = redis.Redis(host=self.addr,port=self.port,password=self.password)
        return r.set('zabbix_monitor','ok')
    @timelimited(2)
    def delete_key(self):
        r = redis.Redis(host=self.addr,port=self.port,password=self.password)
        return r.delete('zabbix_monitor')
...
if __name__ == "__main__" :
    ...
    parser.add_argument('-t','--test',help='redis monitor read write test',action='store_true')
